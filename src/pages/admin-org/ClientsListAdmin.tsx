...
export const ClientsListAdmin: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    return clients.filter(c => {
      const matchSearch =
        ...
        c.nom_responsable.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const totalCA = clients.reduce((s, c) => s + c.montant_paye, 0);
    ...
    <div className="space-y-4 sm:space-y-6 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-2">
          ...
          <span className="text-lg font-bold text-foreground">{fmt(totalCA)}</span>
        </div>
        <div className="rounded-xl border border-border bg-card p-4 space-y-2">
          <div className="flex items-center justify-between">
          ...
          <span className="text-lg font-bold text-foreground">
            {clients.filter(c => c.prochain_renouvellement && new Date(c.prochain_renouvellement) < new Date(Date.now() + 30 * 86400000)).length}
          </span>
        </div>
      </div>
            ...
            {filtered.map((client) => (
              <tr key={client.id} className="hover:bg-muted/30 transition-colors">
                <td className="px-4 py-3">
                  ...
                  <div className="text-xs text-muted-foreground">{client.ville}, {client.pays}</div>
                </td>
                <td className="px-4 py-3 text-foreground">{client.nom_responsable}</td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 font-bold text-foreground">{fmt(client.montant_paye)}</td>
                <td className="px-4 py-3 text-muted-foreground text-xs">{client.prochain_renouvellement || '-'}</td>
                <td className="px-4 py-3">
                  <button
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium text-foreground flex items-center gap-1"><MapPin className="w-3 h-3" />{selectedClient.ville}, {selectedClient.pays}</p>
                </div>
                <div className="space-y-1">
