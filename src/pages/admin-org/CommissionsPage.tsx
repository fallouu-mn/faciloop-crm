...
import { Wallet, TrendingUp, Users, CheckCircle2, ChevronDown, ChevronUp, Calendar } from 'lucide-react';
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-pink-500/10 text-pink-500 text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
              <Wallet className="w-3 h-3" /> Rémunération Variable
            </span>
          </div>
          <p className="text-xs sm:text-sm text-muted-foreground font-semibold mt-0.5">
            Suivi des commissions à verser aux commerciaux — Taux : 5% mensuel, 8% trimestriel, 10% annuel
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <motion.div whileHover={{ y: -3, scale: 1.01 }} className="rounded-3xl border border-border/80 bg-gradient-to-br from-pink-500/5 to-pink-500/10 p-4 sm:p-5 space-y-2 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground">Total à Verser</span>
        ...
        <motion.div whileHover={{ y: -3, scale: 1.01 }} className="rounded-3xl border border-border/80 bg-card p-4 sm:p-5 space-y-2 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground">CA Généré</span>
        ...
        <motion.div whileHover={{ y: -3, scale: 1.01 }} className="rounded-3xl border border-border/80 bg-card p-4 sm:p-5 space-y-2 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground">Commerciaux</span>
        ...
        <motion.div whileHover={{ y: -3, scale: 1.01 }} className="rounded-3xl border border-border/80 bg-card p-4 sm:p-5 space-y-2 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-muted-foreground">Ventes</span>
            <div className="rounded-2xl bg-emerald-500/10 p-2 text-emerald-500 shadow-sm">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground font-semibold">
                        {comm.ventes} vente(s) · CA: {fmt(comm.ca)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      {comm.aVerser > 0 && (
                        <p className="text-[10px] font-bold text-amber-600">{fmt(comm.aVerser)} à verser</p>
                      )}
                    </div>
                    {isOpen ? (
                        ...
                        {comm.entries.map((entry) => (
                          <div key={entry.id} className="p-3 rounded-xl border border-border/60 bg-card flex items-center justify-between gap-3">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${
                              entry.statut === 'a_verser' ? 'bg-amber-500/10 text-amber-600' : 'bg-emerald-500/10 text-emerald-600'
                            }`}>
                              {entry.statut === 'a_verser' ? 'À verser' : 'Versé'}
                            </span>
                          </div>
                        ))}
                      </div>
