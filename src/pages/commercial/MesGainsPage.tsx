...
function formatMonthLabel(monthKey: string): string {
  if (monthKey === 'inconnu') return 'Date inconnue';
...
export function MesGainsPage() {
  ...
  const gainsByMonth = useMemo(() => {
    ...
    return Object.entries(grouped)
      ...
      .map(([monthKey, data]) => ({ monthKey, label: formatMonthLabel(monthKey), ...data }));
  }, [myCommissions]);
        ...
        <div className="relative z-10 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 pt-1">
            <div className="rounded-xl bg-white/10 backdrop-blur-md p-2.5 border border-white/10 text-center">
              <div className="text-xs text-slate-300 font-medium mb-0.5">Mensuel</div>
              <div className="text-xl font-black text-emerald-400">5%</div>
              <div className="text-[9px] text-slate-400">sur chaque mois</div>
            </div>
            <div className="rounded-xl bg-white/10 backdrop-blur-md p-2.5 border border-white/10 text-center">
              <div className="text-xs text-slate-300 font-medium mb-0.5">Trimestriel</div>
              <div className="text-xl font-black text-cyan-400">8%</div>
              <div className="text-[9px] text-slate-400">sur le trimestre</div>
            </div>
            <div className="rounded-xl bg-white/10 backdrop-blur-md p-2.5 border border-white/10 text-center">
              <div className="text-xs text-slate-300 font-medium mb-0.5">Annuel</div>
              <div className="text-xl font-black text-amber-400">10%</div>
              <div className="text-[9px] text-slate-400">sur l'année</div>
            </div>
          </div>

          <p className="text-[11px] text-slate-300 leading-snug">
            Vos gains sont calculés sur le chiffre d'affaires généré par les abonnements souscrits par vos clients.
          </p>
        </div>
      </div>
      ...
      {activeTab === 'apercu' && (
        <div className="space-y-3">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-transparent border border-emerald-500/20 p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground font-medium">Gains totaux accumulés</p>
          ...
          {gainsByMonth.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5">
                <Calendar className="h-3.5 w-3.5 text-primary" /> Gains par période
              </h4>
