...
export const LandingPage: React.FC = () => {
  ...
  const offerFeatures: Record<string, string[]> = {
    ...
    Premium: isEn ? [
      ...
      "(Coming soon) White label",
      "Dedicated account manager",
      "(Coming soon) Multi-organizations",
      "Onboarding & Training",
    ] : [
  ...
  const features = [
    {
      ...
      desc: isEn
        ? 'Adapt the stages of your pipeline to your specific sales process with intuitive drag-and-drop.'
    ...
    },
    {
      ...
      desc: isEn
        ? 'Launch pre-filled WhatsApp conversations in one click from each prospect card.'
        : 'Lancez des conversations WhatsApp pré-remplies en un clic depuis chaque fiche prospect.',
    },
    {
      icon: CalendarClock,
      title: isEn ? 'Smart Follow-ups' : 'Relances Intelligentes',
      desc: isEn
        ? 'Automate your follow-ups and never miss an important follow-up.'
        : 'Automatisez vos relances et ne manquez jamais un suivi important.',
    },
    {
      icon: ShieldCheck,
      title: isEn ? 'Secure Data' : 'Données Sécurisées',
      desc: isEn
        ? 'Your commercial data is isolated and protected, with granular access control by team.'
        : "Vos données commerciales sont isolées et protégées, avec contrôle d'accès granulaire par équipe.",
    },
    {
      icon: BarChart3,
      title: isEn ? 'Sales Dashboard' : 'Tableau de Bord Commercial',
      desc: isEn
        ? 'Track your performance in real-time with clear and actionable charts.'
        : 'Suivez vos performances en temps réel avec des graphiques clairs et actionnables.',
    },
    {
      icon: Building2,
      title: isEn ? 'Simplified Import/Export' : 'Import/Export Simplifié',
      desc: isEn
        ? 'Import your contacts and export your reports in just a few clicks.'
  ...
  // Note: The steps above represent an example of a default pipeline configuration.
  // Your sales pipeline can be fully customized to match your specific sales process.
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-14 max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <LanguageToggle className="ml-2 sm:hidden" />
        ...
        <div className="max-w-5xl mx-auto text-center">
          <FadeInOnScroll>
            ...
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-bold text-foreground leading-tight max-w-4xl mx-auto">
              {isEn ? (
                <>
                  The simple CRM to better follow your prospects and{' '}
                  <span className="text-gradient-faciloop">develop your sales</span>
                </>
              ) : (
                <>
            ...
            <p className="mt-4 sm:mt-6 text-sm sm:text-lg text-muted-foreground max-w-2xl mx-auto font-medium">
              {isEn
                ? 'Centralize your prospects, organize follow-ups, manage your team and pilot your sales from a single platform.'
          ...
          <FadeInOnScroll>
            <div className="text-center mb-8">
              <p className="text-xs font-semibold uppercase tracking-wider text-primary mb-2">
                {isEn ? 'INTERACTIVE SALES PIPELINE' : 'PIPELINE COMMERCIAL INTERACTIF'}
              </p>
              <h2 className="text-xl sm:text-3xl font-bold text-foreground">
                {isEn ? 'Customizable Sales Pipeline in Action' : 'Pipeline Commercial Personnalisable en Action'}
              </h2>
              <p className="mt-2 text-sm text-muted-foreground">
                {isEn ? 'Experience a drag-and-drop pipeline that adapts to your sales process' : "Découvrez un pipeline commercial adaptable à votre processus de vente"}
              </p>
            </div>
          </FadeInOnScroll>
            ...
            <div className="p-4 sm:p-6 rounded-xl border border-border bg-card">
              <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 mb-4">
                {pipelineSteps.map((step, idx) => (
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-full text-xs font-medium transition-all ${demoStep === idx + 1
              ...
              <div className="p-4 rounded-xl bg-muted/50 border border-border">
                ...
                <p className="text-xs text-muted-foreground text-center mt-2">
                  {isEn ? '*The steps shown represent a default example configuration. Your pipeline can be fully customized to match your specific sales process.' : '*Les étapes présentées représentent un exemple de configuration par défaut. Votre pipeline peut être entièrement personnalisé selon votre processus de vente spécifique.'}
                </p>
              </div>
            </div>
          </FadeInOnScroll>
                ...
                <div className="inline-flex items-center rounded-full bg-muted p-1 border border-border text-sm">
                  {PERIODICITES.map((p) => (
                    <button
                      ...
                      className={`px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-medium transition-all ${billingCycle === p.code
                          ? 'bg-gradient-faciloop text-white shadow-sm'
                ...
                <div className="inline-flex items-center rounded-full bg-muted p-0.5 sm:p-1 border border-border shrink-0">
                  {(['XOF', 'EUR', 'USD'] as DeviseCode[]).map((c) => (
                    <button
                      ...
                      className={`px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-medium transition-all ${currency === c ? 'bg-primary text-white shadow-sm' : 'text-muted-foreground'
                        }`}
                    >
                      {c === 'XOF' ? 'FCFA' : c}
                    </button>
            ...
            {formules.map((f, idx) => {
              ...
              return (
                <FadeInOnScroll key={f.code} delay={idx * 100}>
                  <div
                    className={`rounded-xl overflow-hidden border flex flex-col justify-between h-full ${isPopular
                  ...
                  >
                    ...
                    <div className="p-5 sm:p-6 flex flex-col flex-1">
                      ...
                      <div className="mt-6">
                        <Link
                          to="/signup"
                          className={`w-full inline-flex items-center justify-center gap-2 py-3 rounded-full font-semibold text-sm transition-all ${isPopular
          ...
          <FadeInOnScroll>
            <div className="p-6 sm:p-10 rounded-xl border border-border bg-card">
              ...
              <ul className="mt-5 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-foreground">
                {(isEn
                  ? ['Customizable pipeline', 'WhatsApp integration', 'Team collaboration', 'Dedicated support']
                  : ['Pipeline personnalisable', 'Intégration WhatsApp', 'Collaboration d\'équipe', 'Support dédié']
                ).map((item) => (
      ...
      <footer className="border-t border-border bg-background py-10">
        <div className="max-w-5xl mx-auto px-4 text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            {isEn
              ? 'Simple B2B CRM to better follow your prospects and develop your sales.'
              : 'CRM B2B simple pour mieux suivre vos prospects et développer vos ventes.'}
          </p>
      ...
      {/* Floating Language Switcher — Desktop only (mobile is in header) */}
      ...
      <a
        href="https://wa.me/221711387878?text=Bonjour%2C%20je%20souhaite%20en%20savoir%20plus%20sur%20Faciloopro."
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Contacter via WhatsApp"
        className="fixed bottom-4 left-4 z-40 flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#25D366] text-white shadow-lg hover:scale-105 transition-transform"
      >
