              ...
              <tr key={t.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    {t.statut === 'actif' ? (isEn ? 'Active' : 'Actif') : t.statut === 'en_attente' ? (isEn ? 'Pending' : 'En attente') : (isEn ? 'Suspended' : 'Suspendu')}
                  </span>
                </td>
            ...
            <div className="pt-3 border-t border-border flex items-center justify-end">
              {t.statut === 'en_attente' ? (
                ...
                >
                  ...
                  <span>{isEn ? 'Activate' : 'Activer'}</span>
                </button>
              ) : (
                <button
                  onClick={() => toggleStatus(t.id)}
              ...
              {formules.length === 0 && (
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
                  <p className="text-xs text-amber-600 font-medium">
                    {isEn ? 'No plan configured. Go to Subscriptions to create one.' : 'Aucune formule configurée. Rendez-vous dans Abonnements pour en créer.'}
                  </p>
                </div>
              )}
              <div className="space-y-1.5">
